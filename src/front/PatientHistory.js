...
const PatientTreatmentHistory = ({ selectedConsultation = null, onBack }) => {
    ...
    const clientId = selectedConsultation?.clientId;
    ...
    const fetchTreatmentHistory = async () => {
        ...
        try {
            ...
            const response = await api.get(`/clinic/treatment`, {
                params: { clientId: targetId },
                headers: { Authorization: `Bearer ${token}` },
            });

            // Safely unwrap data whether backend returns array directly [] or { records: [] }
            const rawData = response.data?.records ?? response.data;
            const list = Array.isArray(rawData) ? rawData : [];
            console.log(list);

            // Process and sort records chronologically (newest first)
            const sortedList = list.sort((a, b) => {
                const dateA = a.createdAt?.seconds ? new Date(a.createdAt.seconds * 1000) : new Date(a.createdAt || a.date);
                const dateB = b.createdAt?.seconds ? new Date(b.createdAt.seconds * 1000) : new Date(b.createdAt || b.date);
                return dateB - dateA;
            });

            setHistory(sortedList);

            // Auto-expand the most recent record if available
            if (sortedList.length > 0) {
                const firstId = sortedList[0].id || sortedList[0]._id || 0;
                setExpandedCards({ [firstId]: true });
            }
        } catch (err) {
            const errorMsg =
    ...
    return (
        <div className="max-w-5xl mx-auto bg-white border border-slate-200 rounded-2xl shadow-sm p-6 space-y-6 print:p-0 print:border-none print:shadow-none">
            {/* Header */}
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 pb-4 border-b border-slate-100 print:border-b-2 print:border-slate-800">
                <div className="flex items-center gap-3">
                    {onBack && (
                        <button
                            onClick={onBack}
                            className="p-2 border border-slate-200 rounded-lg hover:bg-slate-50 text-slate-600 transition-colors text-xs font-semibold print:hidden"
                        >
                            ← Back
                        </button>
                    )}
                    <div>
                        <h2 className="text-xl font-bold text-slate-800 flex items-center gap-2">
                            <FileText className="text-blue-600 print:text-black" size={24} />
                            Treatment History & Medical Records
                        </h2>
                        <p className="text-xs text-slate-500 mt-0.5">
                            Patient ID: <span className="font-mono text-slate-700 font-semibold">{patientId || clientId || "N/A"}</span>
                        </p>
                    </div>
                </div>
                ...
                <div className="flex items-center gap-2 print:hidden">
                    <button
                        onClick={handlePrint}
                        className="flex items-center gap-1.5 px-3 py-2 border border-slate-200 rounded-lg hover:bg-slate-50 text-slate-700 text-xs font-semibold transition-colors"
                    >
            ...
            {/* Error Alert */}
            {error && (
                <div className="p-4 rounded-xl bg-red-50 border border-red-200 text-red-700 text-xs flex items-center gap-2 print:hidden">
                    <AlertCircle size={16} />
                    {error}
                </div>
            )}
            ...
            {loading ? (
                <div className="py-12 text-center text-slate-400 text-xs print:hidden">
                ...
                <div className="space-y-4 print:space-y-6">
                    {filteredHistory.map((item, index) => {
                        ...
                        return (
                            <div
                                key={cardId}
                                className="border border-slate-200 rounded-xl bg-white shadow-xs overflow-hidden transition-all print:border-slate-300 print:shadow-none print:break-inside-avoid"
                            >
                                {/* Card Top Summary Bar */}
                                <div
                                    onClick={() => toggleCard(cardId)}
                                    className="p-4 bg-slate-50/70 hover:bg-slate-50 cursor-pointer flex flex-col md:flex-row md:items-center justify-between gap-3 border-b border-slate-100 print:bg-slate-100 print:cursor-auto"
                                >
                                    <div className="flex items-center gap-3">
                                        <div className="p-2 rounded-lg bg-blue-100/60 text-blue-700 print:bg-slate-200 print:text-slate-900">
                                            <Calendar size={18} />
                                        </div>
                                        <div>
                                            <div className="flex items-center gap-2">
                                                ...
                                                {item.diagnosisCode && (
                                                    <span className="px-2 py-0.5 rounded bg-blue-50 text-blue-700 font-mono text-[10px] font-bold border border-blue-100 print:border-slate-400 print:text-black">
                                                        {item.diagnosisCode}
                                                    </span>
                                                )}
                                            </div>
                                ...
                                {/* Expanded Card Details (Always visible on print) */}
                                {(isExpanded || window.matchMedia("print").matches) && (
                                    <div className="p-5 space-y-5 bg-white text-xs text-slate-700 print:block">
                                        {/* Doctor Info */}
                                        ...
                                        {item.vitalSigns && (
                                            <div>
                                                <h4 className="font-bold text-slate-800 uppercase text-[10px] tracking-wider mb-2 flex items-center gap-1">
                                                    <Activity size={13} className="text-emerald-600 print:text-slate-800" /> Vital Signs
                                                </h4>
                                                <div className="grid grid-cols-2 sm:grid-cols-5 gap-2 bg-slate-50 p-3 rounded-lg border border-slate-100 print:bg-slate-100">
                                                    <div>
                                        ...
                                        {item.prescription?.length > 0 && (
                                            <div>
                                                <h4 className="font-bold text-slate-800 uppercase text-[10px] tracking-wider mb-2 flex items-center gap-1">
                                                    <Pill size={13} className="text-blue-600 print:text-slate-800" /> Prescribed Medications
                                                </h4>
                                                <div className="border border-slate-200 rounded-lg overflow-hidden">
                                                    <table className="w-full text-left">
                                                        <thead className="bg-slate-50 border-b border-slate-200 text-slate-500 font-semibold text-[11px] print:bg-slate-100">
                                                            <tr>
                                        ...
                                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                            {item.treatment && (
                                                <div>
                                                    ...
                                                    <p className="p-2.5 bg-slate-50 border border-slate-100 rounded-lg text-slate-700 print:bg-slate-100">
                                                        {item.treatment}
                                                    </p>
                                                </div>
                                            )}
                                            ...
                                            {item.tests?.length > 0 && (
                                                <div>
                                                    <h4 className="font-bold text-slate-800 uppercase text-[10px] tracking-wider mb-1 flex items-center gap-1">
                                                        <FlaskConical size={13} className="text-purple-600 print:text-slate-800" /> Requested Lab Tests
                                                    </h4>
                                                    <div className="flex flex-wrap gap-1.5 p-2.5 bg-slate-50 border border-slate-100 rounded-lg print:bg-slate-100">
                                                        {item.tests.map((test, idx) => (
                                                            <span key={idx} className="px-2 py-0.5 bg-purple-50 text-purple-700 border border-purple-200 rounded font-mono text-[10px] print:border-slate-300 print:text-black">
                                                                {test}
                                                            </span>
                                                        ))}
                                                    </div>
                                        ...
                                        {(item.notes || item.followUpDate || item.followUpInterval) && (
                                            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center p-3 bg-blue-50/50 border border-blue-100 rounded-lg text-slate-600 gap-2 print:bg-slate-50 print:border-slate-200">
                                                {item.notes && (
                                                    <p className="italic">
                                                ...
                                                {(item.followUpDate || item.followUpInterval) && (
                                                    <div className="flex items-center gap-1 font-semibold text-blue-800 shrink-0 print:text-slate-900">
